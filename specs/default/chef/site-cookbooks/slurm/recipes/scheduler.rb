...
scripts = ["cyclecloud_slurm.py", "slurmcc.py", "clusterwrapper.py", "cyclecloud_slurm.sh", "resume_program.sh", "resume_fail_program.sh", "suspend_program.sh", "return_to_idle.sh", "terminate_nodes.sh", "update_hosts_file.py"]
...
defer_block "Defer coping /etc/hosts" do
  execute 'Create slurm_hosts' do
     command "cp /etc/hosts /sched/slurm_hosts && rm -rf /etc/hosts && ln -s /sched/slurm_hosts /etc/hosts"
     creates "/sched/slurm_hosts"
  end
end
